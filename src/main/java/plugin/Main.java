...
public class Main extends Plugin {
    ...
    public void registerClientCommands(CommandHandler handler) {
        ...
        handler.<Player>register("teams", "Посмотреть занятые команды.", (args, player) -> {
            playerTeams.each(t->player.sendMessage(t.getTeam().coloredName()));
        });
        handler.<Player>register("spectate", "Перейти в серую команду.", (args, player) -> {
            if (player.team() != Team.derelict) {
                ...
                player.sendMessage("[green]Вы переведены в серую команду");
            } else {
    ...
    public void registerServerCommands(CommandHandler handler) {
        ...
        handler.register("clicks-add", "<uuid>", "добавить игрока в ожидание клика", (args) -> {
            Player fp = Groups.player.find(p->p.uuid().equals(args[0]));
